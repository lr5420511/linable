...
const Linable = module.exports = function(map, callback) {
    this.nodes = Object.assign({}, Linable.preset.naives, Linable.preset.extends);
    this.source = map;
...
Linable.find = function(callback = path => path) {
    ...
    while(path = paths.pop()) {
        ...
        paths = temps.map(coor => [...path, coor]).concat(paths);
        lines = lines.concat(ports.map(cur => [...path, cur]));
    }
